...
def load_data(T=24, nb_flow=1, len_closeness=None, len_period=None, len_trend=None, len_test=None,
    ...
    data, timestamps, data_air = load_stdata('/home/fly/PycharmProjects/version2-baseline-4-28/DeepST-KDD_for_predict/for_submit_data/final_merge_aq_grid_meo_with_weather_normation.h5')
    ...
    Y = []
    timestamps_Y = []
    for data, timestamps, data_air in zip(data_all, timestamps_all, data_air_all):
        # instance-based dataset --> sequences with format as (X, Y) where X is a sequence of images and Y is an image.
        st = STMatrix(data, timestamps, data_air, T, CheckComplete=False)  # 通过将data和timestamp组合创建时空矩阵
    ...
    timestamp_train, timestamp_test = timestamps_Y[:], timestamps_Y[-len_test:]
    X_train = []
    X_test = []
    ...
    # load meta feature
    if meta_data:
        meta_feature = timestamp9vec(timestamps_Y,0)  # 获取长度为9，第8位是工作日标识，9位置是节假日标识
        metadata_dim = meta_feature.shape[1]
        meta_feature_train, meta_feature_test = meta_feature[:], meta_feature[-len_test:]
