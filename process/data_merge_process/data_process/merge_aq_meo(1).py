...
def merge_am():
    ...
    meo_data = pd.read_csv("/home/fly/PycharmProjects/our_competition_project/KDD_air_pollution/data/process_data/18_meo_station.csv")
    # 质量观测站经纬度对应表数据
    aq_station_data = pd.read_csv("/home/fly/PycharmProjects/gmh/18_data_merge_process/data/Beijing_AirQuality_Stations_cn_data.csv")
    ...
    df_meo = pd.DataFrame(meo_data,index=None,columns=['station_id','time','temperature','pressure','humidity','wind_direction','wind_speed','weather'])
    #df_meo.rename(columns={'station_id':'stationName','time':'utc_time','wind_speed':'wind_speed/kph'}, inplace = True)


    df_meo.columns = ['stationId_meo','utc_time','temperature','pressure','humidity','wind_direction','wind_speed/kph','weather']
    ...
    df_merge.to_csv('/home/fly/PycharmProjects/our_competition_project/KDD_air_pollution/data/process_data/merge_aq_meo.csv',index=False)
