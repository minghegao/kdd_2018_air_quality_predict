...
def main():
    ...
    fname_param='/home/fly/PycharmProjects/version2-baseline-4-28/DeepST-KDD_for_predict/scripts/AirPrediction/MODEL/c{len}.p{per}.t{trend}.resunit{res_num}.lr{leaing_ra}.cont.best.h5'.format(len=len_closeness,per=len_period,trend=len_trend,res_num=nb_residual_unit,leaing_ra=lr)
    ...
    y_pred=model.predict([X_train[0][the_predict_index].reshape(1,len_closeness,35,12),X_train[1][the_predict_index].reshape(1,len_period,35,12),X_train[2][the_predict_index].reshape(1,len_trend,35,12),X_train[3][the_predict_index].reshape(1,external_dim)],batch_size=1)
    ...
    pred_array.to_csv('/home/fly/PycharmProjects/version2-baseline-4-28/DeepST-KDD_for_predict/for_submit_data/the_predict_data.csv',index=False)
